...
pub fn spawn_voxel_world(
    ...
    // material(palette index) → cached VoxelMaterial handle（同じ色を何度も addしないため）
    let mut mat_handles: std::collections::HashMap<u16, Handle<VoxelMaterial>> = Default::default();
    ...
    for chunk in chunks {
        for (mat, buf) in chunk.meshes {
            ...
            let translucent = grid.palette.is_translucent(mat);
            let key = mat.0;
            let mat_handle = mat_handles.entry(key).or_insert_with(|| {
                let alpha_mode = if translucent {
                    AlphaMode::Blend
                } else { AlphaMode::Opaque };
                materials.add(VoxelMaterial {
                    color: grid.palette.color(mat).to_linear(),
                    alpha_mode,
            ...
            if grid.palette.is_water_like(mat) {
